...
*   **Measured, not guessed:** `llama3.1:8b` scores **7/8 on facts, 7/8 on tool choice, 0 fabricated ids** over the ten questions in [eval/AGENT.md](../eval/AGENT.md). Good, not trustworthy alone.
*   **It will answer a question from a tool that cannot answer it.** Asked "is there anything that looks like a gas leak?", it reached for `location_hotspots` — aggregate counts with no message text — and reported on absence without ever reading what anyone wrote. There *was* a gas leak, Critical, children inside. Verify anything phrased as "is there any…" by searching yourself.
*   **It says "no results" rather than "that is not recorded"** (0/2 on the eval). Do not read an empty answer as "nobody needs that".
*   **Not deterministic**, despite `temperature: 0` — tool-call paths vary between runs, so the same question can be answered well once and poorly the next time.
